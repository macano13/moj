...
class Subtitles implements SubtitleContract
{
    ...
    {
        // print_r($file_name_or_file_content);
        if (file_exists($file_name_or_file_content)) {
            // print_r("hi");
            return (static::loadFile($file_name_or_file_content));
        }

        return static::loadFile($file_name_or_file_content, $extension);
    }
    ...
    public function srtArrContent()
    {
        $converter = new SrtConverter();
        $content   = $converter->internalFormatToFileContentArray($this->internal_format);
    ...
    public static function loadFile($path, $extension = null)
    {
        ...
        if (empty(mb_detect_encoding($string))) {
...
// if (isset($_POST['file'])) {
//     // print_r($_GET['file']);
//     print_r(Subtitles::load($_POST['file']));
// }
