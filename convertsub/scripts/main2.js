...
function getOriginalText() {
    ...
    for (let n = 0; n < t.length; n++) e[n] = window.Subtitle.parse(t[n].textContent);
    return e
}
...
function saveTranslatedText(e) {
    ...
    l.setAttribute("id", "download-button"), l.setAttribute("class", "red-button translate-button"), l.innerHTML = '<img src="../resources/images/download_cloud.svg" width="24" height="15"/>\n                    <span style="margin-left: 4px;">DOWNLOAD</span>', l.id = "download-button", document.getElementById("button-container").appendChild(l), $(".tool-content-title")[0].innerText = "Download", $(".page-description")[1].innerText = "The translation has finished successfully! Click on the button below to download your file.", $(".goog-te-gadget").hide(), document.getElementsByClassName("steps2")[0].style.backgroundImage = "url('../resources/images/progress_3.svg')"
}
...
$("document").ready(function() {
    ...
    $(".hamburger").click(function() {
        ...
        $("#upload-btn").hide(), $(".table-container").css("display", "block"), $(".tool-nav-container").css("display", "block"), $(".tool-content-title")[0].innerText = "Upload", $(".page-description")[1].innerText = 'Review the files! You can delete or add new files to the list. If is everything all right, just click the "next" button below the\nlist to go to the translation page.', createTable(), updateTable()
        ...
        $("#upload-btn").show(), $(".table-container").css("display", "none"), $(".tool-nav-container").css("display", "none"), $(".tool-content-title")[0].innerText = "Let's Translate!", $(".page-description")[1].innerText = "To get started, click on the button below to upload your files.", $("#upload-form").trigger("reset"), $(".error-container").html(""), $("#files_info_size").css("color", "#616161"), $("#files_info_amount").css("color", "#616161")
        ...
        document.getElementById("pop-up-background").style.display = "block", document.getElementById("pop-up-loading").style.display = "block", $.ajax({
            ...
            type: "POST",
            success: function(e) {
                document.getElementById("pop-up-background").style.display = "none", document.getElementById("pop-up-loading").style.display = "none", window.location.href = "translate.php?id=" + e
            },
            error: function() {
        ...
        if (e = document.getElementsByClassName("goog-te-combo")[0], "" == e.value) alert("Please, Select Language");
        else {
            setTimeout(function() {
                document.getElementById("pop-up-background").style.display = "block", document.getElementById("pop-up-loading").style.display = "block"
            ...
            const n = new MutationObserver(function() {
                if (e = 0, t) {
                    ...
                    let e = setInterval(function() {
                        "FONT" === document.getElementById("text-to-translate").lastChild.lastChild.tagName && isTranslated() && (clearInterval(e), document.getElementById("pop-up-loading").style.display = "none", $(".dialog").show(), n.disconnect())
                    }, 1e3)
                }
            });
        ...
        $(".dialog").hide(), document.getElementById("pop-up-background").style.display = "none", saveTranslatedText(subtitles)
        ...
        e.setAttribute("class", "red-button"), e.setAttribute("id", "new_translation_btn"), e.style.height = "42px", e.style.paddingTop = "0", e.innerText = "NEW TRANSLATION", document.getElementById("button-container").appendChild(e), deleteAll()
    }), $(".tool-content").on("click", "#new_translation_btn", function() {
